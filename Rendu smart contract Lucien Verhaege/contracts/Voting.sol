...
contract Voting is Ownable(msg.sender) {
    ...
    function revokeVote() external onlyAtStatus(WorkflowStatus.VotingSessionStarted) {
    require(voters[msg.sender].isRegistered, "Voter is not registered");
    require(voters[msg.sender].hasVoted, "Voter has not voted yet");

    uint previousVote = voters[msg.sender].votedProposalId;
    voters[msg.sender].hasVoted = false;
    voters[msg.sender].votedProposalId = 0;  // Réinitialise le choix du votant

    proposals[previousVote].voteCount--;  // Diminue le nombre de votes pour la proposition précédente

    emit Voted(msg.sender, 0);  // Émet un événement pour indiquer que le vote a été révoqué
}
}